...
from models.caretaker_model import Caretaker, CaretakerCreate, CaretakerUpdate

def create_caretaker(session: Session, caretaker_in: CaretakerCreate) -> Caretaker:
...
def get_caretaker_by_id(session: Session, caretaker_id: int) -> Caretaker | None:
    return session.get(Caretaker, caretaker_id)

def get_all_caretakers(session: Session, skip: int = 0, limit: int = 100):
...
def update_caretaker(session: Session, db_caretaker: Caretaker, caretaker_in: CaretakerUpdate) -> Caretaker:
