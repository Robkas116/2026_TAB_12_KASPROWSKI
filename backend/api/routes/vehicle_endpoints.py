...
def create_vehicle(
    ...
    """
    try:
        return vehicle_crud.create_vehicle(
            session=db, 
...
def update_vehicle(
    ...
    try:
        return vehicle_crud.update_vehicle(session=db, vehicle_id=vehicle_id, vehicle_in=vehicle_in)
    except IntegrityError:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def delete_vehicle(
    ...
    vehicle_crud.delete_vehicle(session=db, vehicle_id=vehicle_id)
