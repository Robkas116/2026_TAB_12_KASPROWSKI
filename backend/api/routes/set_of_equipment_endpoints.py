...
router = APIRouter(
    prefix="/set-of-equipment",
    tags=["Sets of Equipment"]
)

@router.post("/", response_model=SetOfEquipmentPublic, status_code=status.HTTP_201_CREATED)
def create_set_of_equipment(
    set_of_equipment_in: SetOfEquipmentCreate, 
...
def get_set_of_equipment(
    ...
    db_set = set_of_equipment_crud.get_set_of_equipment_by_id(
        session=db, 
        set_of_equipment_id=set_of_equipment_id
    )
    if not db_set:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
            detail="Set of Equipment not found."
        )
...
@router.post("/{set_of_equipment_id}/equipment/{equipment_id}", response_model=SetOfEquipmentPublic)
def add_equipment_to_set(
    ...
    db_set = set_of_equipment_crud.get_set_of_equipment_by_id(session=db, set_of_equipment_id=set_of_equipment_id)
    if not db_set:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Set of Equipment not found.")
    ...
    return set_of_equipment_crud.add_equipment_to_set(session=db, db_set=db_set, db_equipment=db_equipment)
...
@router.delete("/{set_of_equipment_id}/equipment/{equipment_id}", response_model=SetOfEquipmentPublic)
def remove_equipment_from_set(
    ...
    return set_of_equipment_crud.remove_equipment_from_set(session=db, db_set=db_set, db_equipment=db_equipment)
